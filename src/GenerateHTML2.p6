...
my %t1 = (
     ...
     "Sleeping and Seating" =>    {
        "class" => "tablinks",
      	"Idref" => "sleepingOptions1",
      	'Category' => [
      		{
	    	name => 'Bed Door Options',
	    	values => {
	    	  'Bed Door, Curb Side' => {input => {id=>"slct_bedDoorCS",idref=>"sleepingOptions__bedDoorCS",type=>"checkbox",name=>"model",},
	      	                            label => {id=>"txt_bedDoorCS",class=>"inputText"},
	      		                        },
	      	  'Bed Door, Road Side' => {input => {id=>"slct_bedDoorRS",type=>"checkbox",name=>"model"},
	      		                         },
	      		},
      			},
       		],
...
for my $Menu ( keys %t1 ) {
#   print "Title: $Menu\n";
#   print "Idref: $t1{$Menu}{Idref}\n";
   for my $category ( @{ $t1{$Menu}{Category} } ) {
#      print "   Category: $category->{name} \n";
#      print "   What am I: $category->{values}\n";
      for my $valuekey (keys %{ $category->{values} }){
         print "      values: $valuekey\n";
         for my $values3 (keys %{ $category->{values}->{$valuekey} }){
#           print "         input/label: $values3\n";
           for my $values4 (keys %{ $category->{values}->{$valuekey}->{$values3} }){
             $att[$i_att] = $values4;
#             print "            input/label(attributes): $values4\n";
             $attval[$i_att] = $category->{values}->{$valuekey}->{$values3}->{$values4};
#             print "               attributeValues: $attval[$i_att]\n";
             $i_att ++;
             #print '<input id="slct_bedDoorCS" type="checkbox" idref="sleepingOptions__bedDoorCS" name="model">';
             #print '<label class="inputText" id="txt_bedDoorCS">Bed Door, Curb Side</label><br>' . "\n";
            }
            print "<$values3 ";
            foreach (@att){
              print "$att[$cnt]=\"$attval[$cnt]\" ";
              $cnt ++;
            }
            print ">\n";
            print "\n";
...
sub PrintNav {
  ...
  #
  for my $Menu ( keys %t1 ) {
    print '      <li><a href="#" idref="' . $t1{$Menu}{Idref} . '" class="tablinks">' . $Menu . '</a></li>' . "\n";
  }
  #
  print '    </ul>' . "\n";
  print '  </nav>' . "\n";
...
sub PrintButtonMenu {
	print "\n";
	print '  <div id="main">' . "\n";
	print '    <a><span id="menuIcon" class="glyphicon glyphicon-th-list"></span></a>' . "\n";
	print '    <input id="slct_exteriorViewEnable" type="submit" value="Interior View" class="topButton"><br>' . "\n";
	print '    <input id="slct_nightView" type="submit" value="Night View" class="topButton"><br>' . "\n";
	print '    <input id="slct_collisionRendering" type="submit" value="Enable Collision Rendering" class="topButton"><br>' . "\n";
	print '    <input id="slct_pricingToggle" type="submit" value="Show Price Information" class="topButton"><br>' . "\n";
	print '    <input id="slct_path" type="submit" value="Begin Virtual Tour" class="topButton"><br>' . "\n";
	print '  </div>' . "\n";
	print "\n";
}
...
sub PrintPriceDisplay {
	...
	print '    <button id="priceBreakdownBtn" class="btn btn-default dropdown-toggle" type="button" data-toggle="dropdown"> ' . "\n";
	print '    <span id="menuDownIcon" class="glyphicon glyphicon-menu-down"></span>' . "\n";
	print '    <span id="menuUpIcon" class="glyphicon glyphicon-menu-up"></span></button>' . "\n";
	print '    <ul id="priceBreakdown" class="dropdown-menu">' . "\n";
	print '      <li class="priceComponent">Base Model<p class="componentPrice">$29245</p></li>' . "\n";
	print '      <li class="divider" id="divider"><li>' . "\n";
	print '      <li class="priceComponent">Total:<p id="priceTotalBreakdown" class="componentPrice">$29245</p></li>' . "\n";
	print '    </ul>' . "\n";
	print '  </div>' . "\n";
	print '' . "\n";
}
...
sub PrintX3D {
	...
	print "\n";
	print '    <!-- Scene -->' . "\n";
	print '    <navigationInfo id="head" headlight="true" type=\'"EXAMINE"\'></navigationInfo>'  . "\n";
	print '    <background skyColor=".8 .8 .8 "></background>' . "\n";
	print'    <Viewpoint id="viewPoint" centerOfRotation="0 60 -150" position="289.04603 137.47154 217.53466" orientation="-0.27896 0.95432 0.10702 0.76444" description="defaultX3DViewpointNode"></Viewpoint>' . "\n";
	print "\n";
	print'    <Inline id="dayNightView" nameSpaceName="dayNightView" url="Options/dayNightView.x3d" render="true"></Inline>' . "\n";
	print '    <Inline id="ground" url="x3d_Library/Ground.x3d"> </Inline>'  . "\n";
	print "\n";
	print '    <Inline id="baseModel" nameSpaceName="baseModel" mapDEFToID="true" url="Options/baseModel.x3d" render="true"> </Inline>' . "\n";
	print "\n";
	print '    <!-- Options -->' . "\n";
	print '    <Inline id="sleepingOptions" nameSpaceName="sleepingOptions" mapDEFToID="true" url="Options/sleepingOptions.x3d" render="true"> </Inline>' . "\n";
	print '    <Inline id="heatingAndCoolingOptions" nameSpaceName="heatingAndCoolingOptions" mapDEFToID="true" url="Options/heatingAndCoolingOptions.x3d" render="true"> </Inline>' . "\n";
	print '    <Inline id="interiorOptions" nameSpaceName="interiorOptions" mapDEFToID="true" url="Options/interiorOptions.x3d" render="true"> </Inline> ' . "\n";
	print '    <Inline id="loadingAndTieDownOptions" nameSpaceName="loadingAndTieDownOptions" url="Options/loadingAndTieDownOptions.x3d" render="true"> </Inline>' . "\n";
	print '    <Inline id="exteriorOptions" nameSpaceName="exteriorOptions" mapDEFToID="true" url="Options/exteriorOptions.x3d" render="true"> </Inline>'  . "\n";
	print '    <Inline id="electricalOptions" nameSpaceName="electricalOptions" mapDEFToID="true" url="Options/electricalOptions.x3d" render="true" onClick="console.log(this.id);"> </Inline>'  . "\n";
	print "\n";
	print '    <!--  Other parts -->' . "\n";
	print '    <!--Inline id="tableFoldDown" nameSpaceName="tableFoldDown" mapDEFToID="true" url="Options/tableFoldDown.x3d" render="true"> </Inline-->' . "\n";
	print '    <Inline id="shell" nameSpaceName="shell" mapDEFToID="true" url="Options/shell.x3d" render="true">  </Inline>' . "\n";
	print "\n";
	print '    <!--Inline id="Scene" url="x3d_Library/HadesScene.x3d" render="true"></Inline-->' . "\n";
	print "\n";
	print '    </Scene>' . "\n";
	print '  </X3D>' . "\n";
	print "\n";
...
sub	PrintFormDiv1{
  ...
  print "\n";
    ...
    #print '      <li><a href="#" idref="' . $t1{$Menu}{Idref} . '" class="tablinks">' . $Menu . '</a></li>' . "\n";
     print '    <h3>' . $Menu . '</h3>' . "\n";
        for my $category ( @{ $t1{$Menu}{Category} } ) {
        #print "   Category: $category->{name} \n";
          print '      <h4>' . $category->{name} . '</h4>' . "\n";
          for my $values ( @{ $category->{values} } ) {
            print '      <h4>' . $category->{name} . '</h4>' . "\n";
          }
        }
  }
  ...
  print '    <input id="slct_bedDoorCS" type="checkbox" idref="sleepingOptions__bedDoorCS" name="model"><label class="inputText" id="txt_bedDoorCS">Bed Door, Curb Side</label><br>'
	  . "\n";
  print '    <input id="slct_bedDoorRS" type="checkbox" idref="sleepingOptions__bedDoorRS" name="model"><label class="inputText" id="txt_bedDoorRS">Bed Door, Road Side</label><br>'
	  . "\n";
  print "\n";
  print '    <h4>Happijac Bed Options</h4>' . "\n";
  print '    <input id="slct_happijacMiddle" type="checkbox" idref="sleepingOptions__happijacMiddle" name="model"><label class="inputText" id="txt_happijacMiddle">Happijac Bed, Middle</label><br>'
	  . "\n";
  print '    <input id="slct_happijacRear" type="checkbox" idref="sleepingOptions__happijacRear" name="model"><label class="inputText" id="txt_happijacRear">Happijac Bed, Rear</label><br>'
	  . "\n";
  print "\n";
  print '    <h4>Sofa Options</h4>' . "\n";
  print '    <input id="slct_sofaCSFront" type="checkbox" idref="sleepingOptions__sofaCSFront" name="sofaCSFront"><label class="inputText" id="txt_sofaCSFront">Sleeper Sofa Curb Side Front</label><br>'
	  . "\n";
  print '    <input id="slct_sofaRSMiddle" type="checkbox" idref="sleepingOptions__sofaRSMiddle" name="sofaRSMiddle"><label class="inputText" id="txt_sofaRSMiddle">Sleeper Sofa Road Side Middle</label><br>'
	  . "\n";
  print '    <input id="slct_sofaCSMiddle" type="checkbox" idref="sleepingOptions__sofaCSMiddle" name="sofaCSMiddle"><label class="inputText" id="txt_sofaCSMiddle">Sleeper Sofa Curb Side Middle</label><br>'
	  . "\n";
  print '    <input id="slct_sofaCSRear" type="checkbox" idref="sleepingOptions__sofaCSRear" name="sofaCSRear"><label class="inputText" id="txt_sofaCSRear">Sleeper Sofa Curb Side Rear</label><br>'
	  . "\n";
  print '    <input id="slct_sofaRSRear" type="checkbox" idref="sleepingOptions__sofaRSRear" name="sofaRSRear"><label class="inputText" id="txt_sofaRSRear">Sleeper Sofa Road Side Rear</label><br>'
	  . "\n";
  print "\n";
  print '    <h4>Dinette Options</h4>' . "\n";
  print '    <input id="slct_dinetteCSFront" type="checkbox" idref="sleepingOptions__dinetteCSFront" name="model"><label class="inputText" id="txt_dinetteCSFront">Dinette Curb Side, Front</label><br>'
	  . "\n";
  print '    <input id="slct_dinetteCSMiddle" type="checkbox" idref="sleepingOptions__dinetteCSMiddle" name="model"><label class="inputText" id="txt_dinetteCSMiddle">Dinette Curb Side, Middle</label><br>'
	  . "\n";
  print '    <input id="slct_dinetteCSRear" type="checkbox" idref="sleepingOptions__dinetteCSRear" name="model"><label class="inputText" id="txt_dinetteCSRear">Dinette Curb Side, Rear</label><br>'
	  . "\n";
  print '    <input id="slct_dinetteRSMiddle" type="checkbox" idref="sleepingOptions__dinetteRSMiddle" name="model"><label class="inputText" id="txt_dinetteRSMiddle">Dinette Road Side, Middle</label><br>'
	  . "\n";
  print '    <input id="slct_dinetteRSRear" type="checkbox" idref="sleepingOptions__dinetteRSRear" name="model"><label class="inputText" id="txt_dinetteRSRear">Dinette Road Side, Rear</label><br>'
	  . "\n";
  print "\n";
  print '  </div>' . "\n";
	}
